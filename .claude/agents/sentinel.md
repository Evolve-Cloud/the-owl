---
name: sentinel
description: "Segurança e vulnerabilidades: threat modeling, análise de vulnerabilidades, STRIDE/OWASP, revisão de segurança de código e dependências. Use PROACTIVELY quando o foco é SEGURANÇA (vazamento, injeção, authz, cripto, secrets). NÃO use para qualidade/testes gerais (guardian) nem revisão adversarial de plano/design (challenger)."
