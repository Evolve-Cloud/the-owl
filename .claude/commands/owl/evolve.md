...
Este comando roda como **UMA sessão** (`claude -p "/owl:evolve"`). Os agentes da-owl existem em **duas cópias** (ADR-028): as personas-comando em `.claude/commands/agents/*.md` **e** os subagents nativos em `.claude/agents/*.md` — **13, desde o merge do PR #17.** O loop executa as **personas-comando inline por decisão de confiabilidade do ADR-010**, não por ausência de subagents nativos. Portanto:
...
**L3 — Integrate (por ideia aceita, até o cap).** Para cada aceita:
   ...
   - **@builder (inline `builder.md`)** → aplica UMA edição **lógica** no **`arquivo_alvo`** apontado pela auto-auditoria (o gap concreto), não num alvo genérico. **Se o alvo é uma persona, `arquivo_alvo` é o PAR e a MESMA edição vai nas duas cópias (ADR-028):** `.claude/agents/<x>.md` + `.claude/commands/agents/<x>.md`. Duas cópias mudando em sincronia são **uma** edição expressa duas vezes — não duas mudanças. **Exceção estreita (ADR-034):** para um **campo imposto pela harness** cuja superfície-irmã não impõe equivalente (verificado, não presumido), a edição vai só na superfície que impõe — desde que `arquivo_alvo` liste as duas e marque a excluída com a razão.
   - Manter 1 ideia → 1 ADR → 1 edição **lógica** (atômico).
   - ✅ **Verificar:** o ADR existe e segue `000-template.md`; o diff toca **todos** os arquivos que o `arquivo_alvo` declara **como recebendo a edição**. Para uma persona são normalmente as **duas** cópias, e tocar só uma é **fase FALHOU**, não sucesso parcial (ADR-028). **A verificação chaveia no que o `arquivo_alvo` DECLARA, não numa contagem fixa de dois (ADR-034):** uma cópia marcada como **excluída-com-razão-verificada** conta como satisfeita e não reprova a fase. ⛔ Exclusão **sem** razão verificada registrada = **FALHOU** — a lacuna tem que estar visível no `arquivo_alvo`, nunca omitida.
