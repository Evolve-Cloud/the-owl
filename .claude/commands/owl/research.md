...
1.5. **RETRIEVE-THEN-SEARCH-DELTA — injeção da memória durável (ADR-022).** O codex roda `-s read-only --ephemeral` e **NÃO** pode ler o vault; a retrieval acontece **aqui, no lado da skill**. Antes de montar o prompt final, construir três blocos de injeção e computar duas variáveis:
   ...
   - **REJECTED CLASSES** (marcador `<<INJECT REJECTED CLASSES ...>>`, **ADR-030**): bloco **estático** — sem grep, sem varredura do ledger. A lista de ids decididos ensina o que já **decidimos**; ela não ensina o que nós **somos**, e o padrão dominante de rejeição é por *classe*, não por id (8 menções de "runtime-shaped" no ledger, em 5 ids distintos). Injetar verbatim, sob o título `## REJECTED CLASSES (do NOT propose)`:
     ...
     ```markdown
     ## REJECTED CLASSES (do NOT propose)
     - **Runtime-shaped.** O que desqualifica NÃO é ausência de runtime — é que **o loop não pode mexer no runtime que existe.** Existem spawner (Agent tool + 13 subagents nativos), scheduler (launchd) e hooks de git ativos. Mas o `/owl:evolve` é **sequencial por design** (ADR-010: uma fase por vez, inline, por confiabilidade), e schedule, `.owl/loop-config.yml` e `.claude/settings.json` são **carve-out NFR-SEC-1** — o loop nunca os edita. Ideia que exige trocar o motor de execução, ou que só rende com fan-out paralelo dentro do ciclo, está fora **por fronteira, não por falta de capacidade**. (rejeitados: `isolated-workspaces` 41, `parallel-independent-work` 52, `evaluator-gated-termination` 45.)
     ...
     - **Unenforceable prose.** Convenção que a harness não consegue impor, apresentada como se impusesse, carrega risco de falsa confiança. Esta classe é sobre o que a harness impõe **hoje** — o conjunto do que é enforceable muda quando a plataforma muda. (Instância parcial: `contract-amendment-before-downstream-handoff` 58 — o registro traduz para markdown, a imposição não.)
     ```

     > 📌 **Fonte da verdade das propriedades que este bloco afirma: [`docs/conventions/structural-properties.md`](../../../docs/conventions/structural-properties.md) (ADR-033, forma B).** O registro carrega estado verificado + data + caminhos-prova de cada propriedade. **Mudou lá ⇒ releia aqui** — o acoplamento é nos dois sentidos de propósito: desacoplados, os dois divergem e o defeito "ZERO engine" volta.
     >
     ...
     >
     > 🔧 **Correção 2026-08-07b (human-directed, varredura de propriedades).** "Runtime-shaped" afirmava *"ZERO engine de orquestração: sem spawner, daemon, scheduler ou verificador vivo"*. **Três das quatro negações eram falsas** — verificado contra a árvore: Agent tool + 13 subagents nativos em `.claude/agents/` (spawner), `scripts/com.evolvelabs.owl.daily.plist` (scheduler launchd), `.git/hooks/post-commit` + `post-checkout` **ativos** (superfície de trigger). A conclusão continua certa; a **premissa** que a sustentava não. Reescrita para dizer a capacidade **e** a restrição que de fato desqualifica — o carve-out e a escolha sequencial do ADR-010. **Isto é um filtro mais preciso, não mais fraco:** apagar o "ZERO" sem dizer a fronteira convidaria exatamente as propostas runtime-shaped que este bloco existe para barrar.
     >
     ...
     >
     > O exemplo novo é declaradamente **parcial**: em `contract-amendment-before-downstream-handoff` (58, ciclo 8) a razão *decisiva* foi duplicação — a-owl já emenda contratos por supersessão de ADR — e a inimponibilidade entrou só como queda de Fit secundária. Está citado assim de propósito: o ADR-030 exige uma âncora concreta por classe ("so the model can pattern-match rather than guess"), e uma âncora parcial declarada é melhor que classe abstrata ou que exemplo forte inventado. **Nota:** a própria justificativa do ciclo 8 ancorou nesse `least-privilege-tool-scopes` 66 agora aposentado — o raciocínio de classe se propagou pela âncora velha, que é a razão de manter esta caixa com âncora certa.
