...
2. **Verify the output** (the script prints counts). Confirm:
   - agents = 11 · conventions = 3 · rules files = 3 · skills = 2 · ref ADRs = 7 · meta.yaml = 11
   - `owl-agents/.claude/rules/*.md` are **real files (NOT symlinks)** carrying `paths:` frontmatter — the whole pack must have **0 symlinks** (`find owl-agents -type l` → empty), since it is copied to other machines
