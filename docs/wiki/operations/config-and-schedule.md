...
- `scripts/com.evolvelabs.owl.daily.plist` — the launchd job (daily 07:13).
...
- **The schedule** — `owl-daily.sh` sets PATH (nvm/homebrew), `GIT_SSH_COMMAND` (`~/.ssh/github`), enforces the **shadow-only guard** (refuses to run unless `landing: pr`, unless `OWL_AUTONOMOUS_MAIN=1`), then runs `claude -p --permission-mode bypassPermissions "/owl:evolve"`. launchd fires it daily at 07:13.
...
- **Mac must be awake** at 07:13 (launchd runs at next wake if asleep; missed runs are not stacked).
