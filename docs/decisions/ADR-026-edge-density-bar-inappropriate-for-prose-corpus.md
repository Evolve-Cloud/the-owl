...
**Status:** **Accepted** — **ratificado pelo owner em 2026-08-03**. Este ADR foi autorado por @curator DENTRO do ciclo de verificação do `/owl:evolve` (auto-avaliação) e ficou `Proposed` até a ratificação humana — o sistema não se auto-aprova. O owner ratificou com base em: (1) a barra de 2,0 arestas/nó foi definida para grafos de **código** (call/import densos) e é inapropriada para prosa; (2) o critério correto de "semântico o suficiente" para um corpus de conhecimento é estrutura temática — cumprida (25 comunidades + 14 hubs + 92% EXTRACTED + token cost > 0 + 0 obsidian-dump); (3) a densidade do grafo **não é load-bearing** — a retrieval do retrieve-delta acontece via grep skill-side (`owl/research.md`), provada end-to-end sem depender do grafo, que é camada de navegação secundária; (4) forçar densidade (deep mode) arriscaria regredir as 25 comunidades boas. A barra semântica deste corpus são os quatro critérios acima, não > 2,0.
**Date:** 2026-08-03
