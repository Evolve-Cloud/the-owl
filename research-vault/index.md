...
- [[claude-code-skills-commands-frontmatter]] — a superfície-irmã, e a fonte mais densa do ciclo. Resolveu a pergunta que o ciclo 7 deixou *"for whoever re-opens that id"*: `allowed-tools` **não restringe** (*"every tool remains callable"*), é grant de um turno que alarga. Também revelou que **comandos foram fundidos em skills** ⇒ as 13 personas-comando são skills com ~20 campos disponíveis, e usam **zero**. E foi a frase *"blocks programmatic invocation"* desta página que **reprovou o ADR-036**.
- [[github-copilot-custom-agents]] — `infer: false` (*"Whether the runtime can auto-select this agent"*), verificado porque o brief o afirmou. Convergência de dois fornecedores independentes no mesmo controle, com a mesma âncora de *side-effect*.

> [!note]
> **Uma fonte do brief NÃO tem página, de propósito:** `s1` (Managed Agents / multiagent-orchestration) **não foi buscada** nesta passagem. O SCHEMA manda uma página por fonte **ingerida**; uma fonte não lida não foi ingerida, e criar um stub daria a ela aparência de evidência verificada. É exatamente por isso que `pinned-roster-snapshots` foi deferido (ADR-013) em vez de pontuado com o que o brief afirmou.
