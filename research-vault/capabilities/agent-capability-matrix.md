...
| mcp-builder | camada de **extensões** MCP: Apps (`ui://`, iframe sandboxed, `_meta.ui.csp`/`permissions`), Tasks (handle durável), e o pré-voo negociação → client-matrix → fallback degradado | [[mcp-extensions-apps-tasks]] + `sources/mcp-ext-*` (5 notas) | par `mcp-builder` (ambas as cópias) | **ausente** — `14-mcp-builder-spec-capability` cobre só o core (stateless/MRTR/OAuth), **não** exercita extensions | — (ADR-014/015: sem fixture, impacto é hipótese) | 2026-08-15 |
